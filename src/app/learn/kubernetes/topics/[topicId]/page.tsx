...
export default function KubernetesTopicTeachingPage() {
  ...
  const topic = kubernetesScenarios.find((t) => t.id === topicId);
  ...
  const unlock = topic
    ? getTeachingTopicUnlockState(topic.id, completedIds)
    : { unlocked: false, previousTopicTitle: null, previousTopicId: null };

  const progressId = topic ? topicProgressId(topic.id) : "";
        ...
        <div className="mx-auto max-w-3xl px-6 py-8">
          ...
          <div className={`${scenarioPanel} mt-5 border-gray-700`}>
            ...
            {unlock.previousTopicId && unlock.previousTopicTitle && (
              <Link
                href={`/learn/kubernetes/topics/${unlock.previousTopicId}`}
    ...
    <main className="min-h-screen bg-[#0d1117]">
      <div className="mx-auto max-w-3xl px-6 py-8">
        ...
        <div className={scenarioPanel}>
          ...
          <ul className="mt-2 space-y-1.5 font-mono text-[11px] text-[#3fb950]">
        ...
        <div className={scenarioPanel}>
          ...
          <p className="mt-1 text-[11px] text-gray-500">
            Type the commands above (you can replace{" "}
            <code className="text-[#3fb950]">&lt;pod&gt;</code> with a name like{" "}
            <code className="text-[#3fb950]">hello-k8</code>). Steps above must still be completed in
            order.
          </p>
